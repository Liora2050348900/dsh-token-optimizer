...
### 实测结果
- `outputLadder`：长会话省约 **10% 总输入**（8 个 9k–50k 字符的裸奔输出被采样）。
...
- **dsh-token-optimizer**：管内容，直接省 token（长文转图省 ~72%、输出压缩、MCP 懒加载省 9k/请求、缓存命中率 99.75%）。
- **dsh-behavior-enhancer**：管行为，提升稳定性（失败收敛、并行度恢复、防 OOM 死循环、跨进程还原）。
...
| 模块 | 实测 |
| :--- | :--- |
| text2img | 6,437 字符 → 724 摘要（省 ~72%）；小页分页连续 4 次忠实 |
| outputLadder | 长会话省 ~10% 总输入 |
| mcpLazy | 24 个 MCP 工具 schema ≈ 9,362 token 拦在请求外 |
| 缓存命中率 | 长会话 **99.75%**（缓存价 ≈ 输入价 1/10） |
| compactionDriver | 49 万 token 上下文在核心 0.8 阈值下不触发；本模块 0.45 阈值让其真实触发 |
